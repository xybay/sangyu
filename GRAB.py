...
def BCD_modified(Xtrain, Ytrain, S, lambda_1=1, lambda_2=1, K=5, o_size=.25, max_iter=100, tol=1e-5, dual_max_iter=600,
                 dual_tol=1e-5):
    # 初始化theta
    Theta_init = np.zeros((np.size(Xtrain, 1), np.size(Ytrain, 1)))
    P = Theta_init.shape[1]  # P=90
    ...
    for i in range(max_iter):
        ...
        # train_ll = 0.5 * (
        #     np.power(np.linalg.norm(np.matrix(Ytrain) - np.dot(np.matrix(Xtrain), np.matrix(Theta)), ord='fro'), 2))
        # +lambda_2 * np.linalg.norm(np.matrix(Theta), ord='nuc') + lambda_1 * np.sum(np.abs(Theta))
        # + lambda_1 * np.trace(lmbdas * np.abs(Theta))
        train_ll = fval_vec[-1]
...
def accel_grad_mlr_qiao_modified(Xtrain, Ytrain, lambdas, lambda_2, Theta0):
    ...
    while np.abs(fval_old - fval) > epsilon:
        ...
        while fval > Q:
            ...
            (Wp, P, sval) = ComputeQP(Xtrain, Ytrain, Z_old, L, lambdas, lambda_2)
            f = 0.5 * (
                np.power(np.linalg.norm(np.matrix(Ytrain) - np.dot(np.matrix(Xtrain), np.matrix(Wp)), ord='fro'), 2))
            fval = f + sval
            Q = P + sval
            # print ("fval-Q: ", fval-Q)
...
def ComputeQP(X, Y, W, L, lambdas, lambda_2):
    ...
    P = 0.5 * (np.power(np.linalg.norm(np.matrix(Y) - np.dot(np.matrix(X), np.matrix(W)), ord='fro'), 2)) + np.trace(
        np.dot(np.matrix(delta_W.T), np.matrix(Wp - W))) + 0.5 * L * (np.power(np.linalg.norm(Wp - W, ord='fro'), 2))
    return (Wp, P, sval)
